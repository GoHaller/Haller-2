      ...
      columns: [
         ...
         },
        {
          "data": "action", "mRender": function (data, type, row) {
            if (row.isBlocked) {
              var actionhtml = '<button type="button" rel="tooltip" class="btn btn-success block" data-status=0 data-id="' + row.id + '" ">UnBlock</button>';
            } else {
              var actionhtml = '<button type="button" rel="tooltip" class="btn btn-danger  block" data-status=1 data-id="' + row.id + '" ">Block</button>';
            }
            return actionhtml;
          }
        }
