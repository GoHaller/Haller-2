...
export class BotComponent implements OnInit {
    ...
    getAllUserWithFilter() {
        this.userService.getUsersListWithFilter(this.skip, this.limit, this.searchkey || null)
            .subscribe((res) => {
                this.userList = res;
            }, error => {
    ...
    getConversationForRecipient() {
        this.userService.getConversationForRecipient(this.selectedUser['_id'])
            .subscribe((res: any) => {
                if (res && res['_id']) { this.conversation = res; } else { this.conversation = {}; }
            ...
            res.result.fulfillment.messages.forEach(msg => {
                if (!msg.platform) {
                    if (msg.type == 0) {
                        ...
                        if (msg.payload.link) {
                            botConvo.push({
                                id: res['id'], link: msg.payload.link,
                                linkText: msg.payload.linkText || msg.payload.link
                            });
                        }
                        if (msg.payload.location) {
                            botConvo.push({
                                id: res['id'], location: msg.payload.location,
                                linkText: msg.payload.linkText || 'Show on map'
                            });
                        }
                        if (msg.payload.address) {
                            botConvo.push({ id: res['id'], body: msg.payload.address });
                            botConvo.push({
                                id: res['id'], address: msg.payload.address,
                                linkText: msg.payload.linkText || 'Show on map'
                            });
                        }
                    }
